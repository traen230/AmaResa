...
# -----------------------------------
# メイン処理関数（AmazonカテゴリランキングURLを受け取る）
# -----------------------------------
def run_rival_check_ranking(url):
    driver = None
    try:
        # --- ChromeDriverと拡張機能のパス ---
        ...
        for i, item in enumerate(items):
            ...
            output.append([
                rank, title, product_url, review_count, rating, sales, remark
            ])
        ...
        with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.writer(f)
            writer.writerow(["順位", "商品名", "商品URL", "評価数", "評価値", "直近30日販売数（親）", "備考"])
