# ✅ 修正版 ranking_research.py（戻るボタン対応：即時実行に戻して安定表示）
...
class Ranking_ResearchWindow(QWidget):
    ...
    def go_back(self):
        if self.main_window:
            try:
                from market_screen import MarketResearchScreen  # 🔄 遅延インポートで循環参照回避
                screen = MarketResearchScreen(parent=self.main_window)
                self.main_window.show_screen(screen, "🔙 市場リサーチメニューに戻りました。")
            except Exception as e:
                if hasattr(self.main_window, "append_log"):
                    self.main_window.append_log(f"❌ 戻り時にエラー発生: {e}")
